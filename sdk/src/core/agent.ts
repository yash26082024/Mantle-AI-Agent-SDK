...
import type { AgentConfig, ExecutionResult, ReasoningResult } from "../types";
...
export class Agent {
   ...
   */
  async runIteration(systemPrompt: string): Promise<ExecutionResult> {
    try {
      // 1. Observe
      const observationData = await this.observer.observe();

      // 2. Decide (Reason)
      const decision: ReasoningResult = await this.llm.reason(systemPrompt, observationData);
      ...
      const totalValue = sanitizedActions.reduce(
        (acc, action) => acc + BigInt(action.value),
        BigInt(0)
      );

      const tx = await this.executorContract.getFunction("execute")(
        sanitizedActions,
        this.config.agentId,
        reasoningHash,
        { value: totalValue } // Send the required MNT with the call
      );
      ...
      return {
        ...
        results: receipt.logs.map((log: any) => log.data)
      };
    } catch (error: any) {
